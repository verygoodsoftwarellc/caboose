  ...
  class BackoffPolicy
    ...
    def initialize(options = {})
      @min_timeout_ms = options.fetch(:min_timeout_ms) {
        ENV.fetch("CABOOSE_BACKOFF_MIN_TIMEOUT_MS", MIN_TIMEOUT_MS).to_i
      }
      @max_timeout_ms = options.fetch(:max_timeout_ms) {
        ENV.fetch("CABOOSE_BACKOFF_MAX_TIMEOUT_MS", MAX_TIMEOUT_MS).to_i
      }
      @multiplier = options.fetch(:multiplier) {
        ENV.fetch("CABOOSE_BACKOFF_MULTIPLIER", MULTIPLIER).to_f
      }
      @randomization_factor = options.fetch(:randomization_factor) {
        ENV.fetch("CABOOSE_BACKOFF_RANDOMIZATION_FACTOR", RANDOMIZATION_FACTOR).to_f
      }
