...
module Caboose
  class SpansController < ApplicationController
    ...
    def show
      @span = Caboose.storage.find_span(params[:id])
    ...
    def list_spans(category)
      ...
      spans = Caboose.storage.list_spans_by_category(category, **filter_params, limit: PER_PAGE + 1, offset: @offset)
      @total_count = Caboose.storage.count_spans_by_category(category, **filter_params)
      ...
      if span_ids.any?
        all_properties = Caboose.storage.load_properties_for_ids("Caboose::Span", span_ids)
        @spans.each do |span|
